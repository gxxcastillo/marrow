...
export async function writeReadme(toolRoot: string, agentsPath: string, project: string, branch: string): Promise<void> {
  const template = async (name: string) =>
    (await readFile(path.join(toolRoot, "templates", name), "utf8"))
      .replaceAll("{{project}}", project)
      .replaceAll("{{branch}}", branch);
  const block = await template("persistence-block.md");
  const readmePath = path.join(agentsPath, "README.md");

  if (!existsSync(readmePath)) {
    await writeFile(readmePath, `${await template("readme-seed.md")}\n${block}`);
    return;
  }
